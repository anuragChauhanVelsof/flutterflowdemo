...
class TaskTrackerSupabaseUser extends BaseAuthUser {
  TaskTrackerSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> taskTrackerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TaskTrackerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
